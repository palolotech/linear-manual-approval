...
async function run(): Promise<void> {
  ...
  const teams = (await linearClient.teams({filter: {name: {eq: teamName}}}))
    .nodes

  const ticketsTeam = teams.find(team => team.name === teamName)
  if (ticketsTeam === undefined) {
    throw new Error(`${teamName} team not found`)
  }

  const linearIssueCreateOptions: IssueCreateInput = {
    title: issueTitle,
    teamId: ticketsTeam.id,
  ...
  if (startingStatus !== 'do not set') {
    ...
    const statuses = (
      await linearClient.workflowStates({
        filter: {team: {id: {eq: ticketsTeam.id}}, name: {eq: startingStatus}}
      })
    ).nodes
  ...
  console.log(
    `Created issue: ${issue.id} at ${issue.createdAt} located at URL ${issue.url}`
  )
  ...
  while (true) {
    ...
    const updatedIssue = await linearClient.issue(issue_id)
